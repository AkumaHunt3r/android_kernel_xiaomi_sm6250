...
/**
 * kthread_run_non_perf_critical - create and wake a non performance-critical thread.
 *
 * Same as kthread_run(), but with the kthread bound to low power CPUs.
 */
#define kthread_run_non_perf_critical(threadfn, data, namefmt, ...)		   \
({									   \
	struct task_struct *__k						   \
		= kthread_create(threadfn, data, namefmt, ## __VA_ARGS__); \
	if (!IS_ERR(__k)) {						   \
		__k->flags |= PF_PERF_NON_CRITICAL;				   \
